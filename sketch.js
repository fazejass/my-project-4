...
if(rabbitGroup.isTouching(bulletGroup)){
 ...
 for(var i=0;i<rabbitGroup.length;i++){     
  ...
  if(rabbitGroup[i].isTouching(bulletGroup)){
    rabbitGroup[i].destroy()
...
if(rabbitGroup.isTouching(player)){
  ...
  for(var i=0;i<rabbitGroup.length;i++){     
   ...
   if(rabbitGroup[i].isTouching(player)){
        rabbitGroup[i].destroy()
        } 
...
function enemy(){
  if(frameCount%50===0){
    ...
    rabbit = createSprite(random(1600,1900),random(200,1000),40,40)

    rabbit.addImage(rabbitImg)
    rabbit.scale = 0.4
    rabbit.velocityX = -3
    rabbit.debug= false
    rabbit.setCollider("rectangle",0,0,400,400)
